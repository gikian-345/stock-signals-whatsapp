...
from messenger import send_whatsapp_message
...
def build_message(date_ny: dt.datetime, picks: list[dict], others: list[dict], top_n: int = 15) -> str:
    ...
    header += "Universe: S&P 500 + Nasdaq-100 | Signals: SMA20/50, RSI(14), 52w proximity, volume spike\n\n"
    lines = []
    if picks:
    ...
    if sells:
        ...
        lines.append("")
    footer = "—\nEducational only. Tune rules in repo to fit your strategy."
    return header + "\n".join(lines) + footer
